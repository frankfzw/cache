...
simulate_access(VOID *addr, UINT32 access_type)
{
        ...
        hit1 = avdc_fetch(avdc1, pa);
        if (hit1)
        {
        	avdc_access(avdc1, pa, type);
        }
        else 
        {
        	hit2 = avdc_fetch(avdc2, pa);
        	if (hit2)
        	{
        		avdc_access(avdc2, pa, type);
        		avdc_invalid(avdc2, pa);
        	}
        	avdc_access(avdc1, pa, type);
        	...
        	if (avdc1->victim != 0)
        	{
        		avdc_access(avdc2, avdc1->victim, type);
        	}
        }
