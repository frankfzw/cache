...
int 
two_level_access(avdark_cache_t *l1, avdark_cache_t *l2, avdc_pa_t pa, avdc_access_type_t type)
{
	int hit1, hit2 = 0;
        int ret = 0;
        
        hit1 = avdc_access(l1, pa, type);
        
        if (!hit1)
        {
        	hit2 = avdc_access(l2, pa, type);
        	...
        	if (l1->victim_flag != 0)
        	{	
        		avdc_dbg_log(l1, "victim: %lx\n", (unsigned long)l1->victim);
        		avdc_access(l2, l1->victim, type);
